# https://github.com/modularml/mojo/blob/main/stdlib/src/builtin/coroutine.mojo
# ===----------------------------------------------------------------------=== #
...
struct _CoroutineContext:
    ...
    alias _resume_fn_type = fn (Self._opaque_handle, Self._opaque_handle) -> None
...
struct Coroutine[type: AnyRegType]:
    ...
    fn _get_promise(self) -> Pointer[type]:
        ...
        """
        var promise: Pointer[Self._promise_type] = __mlir_op.`pop.coroutine.promise`(
            self._handle
        )
