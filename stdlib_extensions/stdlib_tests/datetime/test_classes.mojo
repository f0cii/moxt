...
def test_datetime_min_max():
    assert_equal(datetime.min().__str__(), py_datetime().min.__str__())
    assert_equal(datetime.max().__str__(), py_datetime().max.__str__())
...
def test_datetime_timedelta_interaction():
    ...
    for i in range(1_000):
        ...
        assert_equal(mojo_dt.__str__(), py_dt.__str__())
        # assert_equal(mojo_dt.__repr__(), py_dt.__repr__())
...
def test_date():
    ...
    assert_equal(
        simple_date.__str__(),
        Python.import_module("datetime").date(2020, 1, 1).__str__(),
    )

    # we'd be extremely unlucky if this fails
    assert_equal(date.today().__str__(), py_date().today().__str__())
...
def test_time():
    ...
    assert_equal(
        simple_time.__str__(),
        Python.import_module("datetime").time(12, 30).__str__(),
    )
